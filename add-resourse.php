...
<?php 
$uid=$_SESSION['warroomuid'];
$query=mysqli_query($con,"SELECT rs.id,rs.bloffice,rs.flmoffice,rs.gentype,rs.gensource,rs.genslno,rs.gencondition,rs.postingDate,rs.updationDate from resourse as rs LEFT JOIN tbluser usr ON rs.officeid=usr.officeid where usr.id='$uid'ORDER BY rs.id asc");
$cnt=1;
...
?>								
										<tr>
											...
											<a href="updateresourse.php?rid=<?php echo $row['id']?>" data-toggle="tooltip" data-original-title="Edit"><i class="icon-edit" style="color:orange"></i></a><font style="color:white"> |<>| </font>
											<a href="add-resourse.php?rid=<?php echo $row['id']?>&del=delete" onClick="return confirm('Are you sure you want to Delete this Resource?')"><i class="icon-remove-sign" style="color:red"></i></a>
											</td>
